...
output_dir <- paste0("./orinoquia_processing/extracted_features")
...
pu_raster <- raster(paste0("./PUs/PUs_", extension, "_", resolution, ".tif"))
...
# 4  - Páramo
# 7  - Bosque seco
# 20 - Especies Focales (~40 species - will aggregate to richness)
# 21 - Especies (~8700 species - will aggregate to richness)
# 11 - Carbono orgánico en suelos
# 12 - Biomasa aérea más biomasa subterránea
# 16 - Areas NucleoSIRAPO
# 17 - HumedalesCOL_EC
# 19 - Congriales
# 23 - Condor Habitat

primary_features <- c(1, 4, 7, 20, 21, 11, 12, 16, 17, 19, 23)
...
for (i in 1:nrow(feature_name_map)) {
  ...
  if (!is.na(feat_source_file)) {
    ...
    if (feat_id_elemento == 21 || feat_id_elemento == 20) {
