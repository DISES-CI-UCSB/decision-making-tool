...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Herramientas del frontend   | Node.js 22 (CI), Yarn 4.18.0 (declarado en `frontend/package.json`). Compilación de producción mediante `yarn build:vercel` desde `frontend/`.           | ✅ Verificado                                                                                                                                          |
...
| Almacenamiento              | Vercel Blob medido el 18 sep 2026: **364 GiB** en 52.347 archivos (GiB = gibibyte, 1024³ bytes). Unos 350 GiB son `releases/` históricas; el conjunto de trabajo actual sigue siendo de decenas de GiB (una versión de soluciones es ~75–83 GiB). Firestore es un almacén de autorización gestionado y pequeño, no este inventario. | ✅ Inventario verificado; 🟠 TI de Parques aún decide la retención de versiones históricas y si Blob público sigue siendo aceptable.    |
