...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Frontend toolchain | Node.js 22 (CI), Yarn 4.18.0 (declared by `frontend/package.json`). Production build via `yarn build:vercel` from `frontend/`.                            | ✅ Verified                                                                                                          |
...
| Storage            | Vercel Blob measured 18 Sep 2026: **364 GiB** across 52,347 files (GiB = gibibyte, 1024³ bytes). About 350 GiB is historical `releases/`; a current working set is still tens of GiB (one solutions release is ~75–83 GiB). Firestore is a small managed auth store, not this inventory. | ✅ Verified inventory; 🟠 Parques IT still decides retention of historical releases and whether public Blob stays acceptable. |
