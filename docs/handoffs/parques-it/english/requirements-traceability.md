...
- Do not claim all runtime dependencies are fully locked — npm has a lockfile, but Python requirements still use minimum-version ranges. CI tests Python 3.11 and 3.12; the Dockerfile is still `python:3.11-slim`. Exact dependency locking remains future work.
