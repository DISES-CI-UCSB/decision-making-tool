...
Read the status column first. Open decisions live in the Top decisions table. This page is not a runbook: architecture, cybersecurity, usability, and performance have their own pages, and Data Operations is an index of operator procedures.
...
| REQ-06     | Interoperability & Institutional | Explain institutional customization and compliance with government application standards.                  | 🟠 Parques IT decision required | Product branding exists on main (EcoPlan header logo, partner carousel, /about). Still open: Gobierno Digital matrix, official PNNC mark rules, empty SIRAP logo slot.                            |
...
| REQ-10     | Documentation & Manuals          | Document functional use cases.                                                                             | 🟡 Team confirmation required   | Shipped analysis, admin, and Finder workflows are inventoried in [`usability-testing.md`](./usability-testing.md). Formal role-based use-case documents and manuals still do not exist.           |
...
| REQ-15     | Documentation & Manuals          | Create a technical manual declaring language and development-package versions.                             | 🟡 Team confirmation required   | English E3-FO-35 set is in [`reception/`](./reception/): install, technical requirements, administration, users, data model, FAQ/troubleshooting. Backup/restore/upgrade and a single Spanish operations book are still open. |
...
- **SIRAP models:** UI-certified catalog is delivered — 56 published scenarios (40 Eje Cafetero, 16 Orinoquía). Remaining sign-off is carbon units and IHEH2030 provenance, not package delivery. Keep this distinct from whether national Prioritizr execution code is reproducible.
...
- Do not claim all runtime dependencies are fully locked — Yarn has a lockfile, but Python requirements still use minimum-version ranges. CI tests Python 3.11 and 3.12; the Dockerfile is still `python:3.11-slim`. Exact dependency locking remains future work.
- Do not claim formal end-to-end tests exist — current automated evidence is unit, contract, manifest, fixture, and narrow browser smoke tests. As of 2026-09-07 the suite under `backend/tests/` contains 165 tests. Arbitrary custom-AOI category-mask behavior still needs a targeted production-path regression test; see [`performance-testing.md`](./performance-testing.md#backend-fixture-correction-and-remaining-production-concern).
...
- Do not claim SIRAP model packages have not been delivered, or that the SIRAP solution area is future or empty — 56 published scenarios exist (40 Eje Cafetero, 16 Orinoquía). Remaining SIRAP work is scientific sign-off (carbon units, IHEH2030 provenance), not catalog existence. This is separate from national Prioritizr reproducibility, which still requires confirmation.
...
- REQ-15 now has an English reception package in [`reception/`](./reception/). Depth still lives in [`architecture.md`](./architecture.md), [`cybersecurity.md`](./cybersecurity.md), and [`performance-testing.md`](./performance-testing.md). A single Spanish operations book, plus backup/restore/upgrade procedures, are still open. Functional user manuals remain REQ-16.
...
- Product branding exists on main (EcoPlan header logo, partner carousel, /about). Still open: Gobierno Digital matrix, official PNNC mark rules, empty SIRAP logo slot. Do not infer Gobierno Digital compliance from bilingual support alone.
