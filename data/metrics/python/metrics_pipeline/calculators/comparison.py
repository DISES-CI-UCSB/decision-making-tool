"""Pairwise scenario comparison metrics.
...
They are intended for:
  ...
  - Live frontend calculations when two scenarios are loaded side-by-side.
...
------------------------
  ...
  #71 — Unique to Scenario A  (A − B)
  #72 — Unique to Scenario B  (B − A)
"""
...
def agreement_area_km2(raster_a: SolutionRaster, raster_b: SolutionRaster) -> float:
    """#70 — km² selected in BOTH scenario A and scenario B (A ∩ B)."""
...
def unique_to_a_km2(raster_a: SolutionRaster, raster_b: SolutionRaster) -> float:
    """#71 — km² selected in scenario A but NOT in scenario B (A − B)."""
    _require_same_grid(raster_a, raster_b)
...
def unique_to_b_km2(raster_a: SolutionRaster, raster_b: SolutionRaster) -> float:
    """#72 — km² selected in scenario B but NOT in scenario A (B − A)."""
    _require_same_grid(raster_a, raster_b)
