...
| Folder | What's in it |
|--------|-------------|
| `inputs/` | Solver inputs grouped by role: `features/`, `costs/`, `includes/` |
...
| File | Purpose |
|------|---------|
| `input_layers_in_use.csv` | Non-solution layers currently available — name, type, path, app status |
| `input_layers_required.csv` | Non-solution layers still needed or not fully wired |
...
`layer_group` uses `features`, `cost`, and `includes` for solution inputs; use `boundaries` and `context_overlays` for non-input context layers.

### Layers In Use Snapshot (synced from `input_layers_in_use.csv`)
...
| `INCL_RUNAP` | Protected Areas (RUNAP 2023) | includes | include | nacional | true | true | `data/inputs/includes/runap_protected_areas.tif` | Always applied include |
| `INCL_COMUNIDADES` | Afro-Colombian Communities | includes | include | nacional | true | true | `data/inputs/includes/comunidades.tif` | Optional include toggle |
| `INCL_RESGUARDOS` | Indigenous Reserves (Resguardos) | includes | include | nacional | false | false | `data/inputs/includes/resguardos.tif` | Data exists but not wired in UI |
| `INCL_OMECS` | OMECs (raster) | includes | include | nacional | true | true | `data/inputs/includes/omecs.tif` | Optional include toggle |
| `INCL_OMECS_VECTOR` | OMECs (vector) | includes | reference | nacional | false | false | `data/inputs/includes/omecs_vector/` | Vector overlay source |
...
### Layers Required Snapshot (synced from `input_layers_required.csv`)
...
| 2 | `inputs/includes/runap_protected_areas.tif` | Include |
| 3 | `inputs/includes/comunidades.tif` | Include |
| 4 | `inputs/costs/net_benefit.tif` | Cost |
| 5 | `inputs/includes/resguardos.tif` | Include |
| 6 | `inputs/includes/omecs.tif` | Include |
